...
module.exports = class Databases {
    ...
    static createFromSensorList(sensors) {
        var retval = new Databases();
        if (sensors !== undefined) {
            sensors.forEach(sensor => {
                var path = sensor['path'];
                var databases = sensor['databases'];
                (sensor['databases'].split(' ')).forEach(name => {
                    var database = retval.getList().reduce((a,database) => ((database.getName() == name)?database:a), null);
                    if (database != null) {
                        database.addPath(path);
                    } else {
                        retval.add(new Database(name, [path]));
                    }
                });
            });
        }
