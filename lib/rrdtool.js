...
const PERIODS = [
    { name: "hour", tag: "end-1h", seconds: 3600, consolidate: "MAX" },
    { name: "day", tag: "end-1d", seconds: 86400, consolidate: "MAX" },
    { name: "week", tag: "end-1w", seconds: 604800, consolidate: "AVERAGE" },
    { name: "month", tag: "end-1m", seconds: 2592000, consolidate: "AVERAGE" },
    { name: "year", tag: "end-1y", seconds: 31104000, consolidate: "AVERAGE" }
];

var USE_SERVER = false;
var SERVER_SOCKET = null;
var SERVER_LOGLEVEL = false;

exports.useServer = function(host, port, loglevel) {
    USE_SERVER = true;
    SERVER_SOCKET = new net.Socket();
    SERVER_LOGLEVEL = (loglevel === undefined)?0:loglevel;

    SERVER_SOCKET.on('connect', function() {
    });

    SERVER_SOCKET.on('close', function() {
        if (SERVER_LOGLEVEL != 0) console.log('rrdtool: server connection closed');
        SERVER_SOCKET = null;
    });

    SERVER_SOCKET.on('data', function(data) {
        switch (SERVER_LOGLEVEL) {
            case 1:
                if (data.includes("ERROR")) console.log('rrdtool: received: ' + data);
                break;
            case 2:
                console.log('rrdtool: received: ' + data);
                break;
            default:
                break;
        }
    ...
    SERVER_SOCKET.connect(port, host);
}
...
exports.createDatabase = function(filename, interval, min, max, dsnames) {
    ...
    var command = ((USE_SERVER)?"":(RRDTOOL + " ")) + "create '" + filename + "'";
...
exports.updateDatabase = function(filename, dsnames, dsvalues) {
    ...
    var command = RRDTOOL;
    var command = ((USE_SERVER)?"":(RRDTOOL + " ")) + "update " + filename;
...
exports.createChart = function(dirname, filename, dsnames, options) {
    ...
    var command = ((USE_SERVER)?"":(RRDTOOL + " ")) + "graph '" + dirname + optPeriod + "." + optImgFormat.toLowerCase() + "'"
    command += " -T 80";
    ...
    command += " --height=300";
    dsnames.forEach(function(dsname) {
        command += " DEF:" + dsname + "=" + filename + ":" + dsname + ":" + PERIODS.reduce((a,v) => ((v['name'] == optPeriod)?v['consolidate']:a),PERIODS[0]['consolidate']);
        ...
        command += " CDEF:" + dsname + "filled=" + dsname + ",UN,PREV," + dsname + ",IF";
